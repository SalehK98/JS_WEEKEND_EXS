...
// solution by keeping how many time we preformed the equation 
// unless percent is zero we do not count the last iteration because 
...
function nbYear(p0, percent, aug, p) {
    ...
    if (p0 >= p) return 0
    if (percent === 0 || null) {
        while (Math.floor(sum) < p) {
            // console.log(sum, Math.floor(sum), p, percent, aug)
            sum = sum + aug
            count++
        }
    } else {
...
console.log(nbYear(1500, 5, 100, 5000)) // expected output -> 15 (v)
console.log(nbYear(1500000, 0.0, 10000, 2000000)) // expected output -> 10 (v)